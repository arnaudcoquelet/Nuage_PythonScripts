...
echo "*********************************************" &>> $BACKUP_LOG
echo "" &>> $BACKUP_LOG
echo "innobackupex --user=root $BACKUP_DIR" &>> $BACKUP_LOG
innobackupex --user=root $BACKUP_DIR &>> $BACKUP_LOG
...
then
   ...
   echo "*********************************************" &>> $BACKUP_LOG
   echo "innobackupex --user=root --apply-log --use-memory=4G $CURRENT_BACKUP" &>> $BACKUP_LOG
   echo "" &>> $BACKUP_LOG

   innobackupex --user=root --apply-log --use-memory=4G $CURRENT_BACKUP &>> $BACKUP_LOG
